...
public static class DOS2DEPackageCreator
{
	...
	public static async Task<bool> CreatePackage(string dataRootPath, List<string> inputPaths, string outputPath, List<string> ignoredFiles, CancellationToken? token = null)
	{
		try
		{
			...
			var package = new PackageBuildData()
			{
				Version = Game.DivinityOriginalSin2DE.PAKVersion(),
	...
	private static Task AddFilesToPackage(PackageBuildData package, string path, string dataRootPath, string outputPath, List<string> ignoredFiles, CancellationToken token)
	{
		Task? task = null;
		...
		task = Task.Run(() =>
		{
			...
			var files = Directory.EnumerateFiles(path, "*", DefaultResources.RecursiveOptions)
			.Where(f => !ignoredFiles.Any(x => IgnoreFile(f, x)))
	...
	private static Task WritePackage(PackageWriter writer, PackageBuildData package, string outputPath, CancellationToken token)
	{
