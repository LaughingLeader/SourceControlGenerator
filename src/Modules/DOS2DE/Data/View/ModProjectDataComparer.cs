...
public class ModProjectDataComparer : ICustomSorter
{
	...
	private static readonly List<string> StringTypes =
	[
		"ModuleInfo.Name",
	...
	public int Compare(object x, object y)
	{
		if (x is ModProjectData a && y is ModProjectData b)
		{
			...
			else if (StringTypes.Contains(SortMemberPath))
			{
				...
				var property = SortMemberPath.Replace("ModuleInfo.", "");
				var valueA = (string)dataType.GetProperty(property).GetValue(a.ModuleInfo);
				var valueB = (string)dataType.GetProperty(property).GetValue(b.ModuleInfo);
