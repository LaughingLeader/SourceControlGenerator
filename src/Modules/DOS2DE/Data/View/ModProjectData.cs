...
public class ModProjectData : ReactiveObject, IProjectData, IDisposable
{
	...
	*/
	...
	private void LoadThumbnail(string projectDirectory)
	{
		...
		{
			thumbnail = Directory.EnumerateFiles(projectDirectory, "thumbnail", DefaultResources.FlatSearchOptions).FirstOrDefault();
		}

		if (!string.IsNullOrWhiteSpace(thumbnail) && FileCommands.IsValidImage(thumbnail))
		{
	...
	#region Async
	public async Task<Unit> LoadModMetaAsync(string metaFilePath)
	{
		if (File.Exists(metaFilePath))
		{
			...
			var contents = XMLHelper.EscapeXmlAttributes(await FileCommands.ReadFileAsync(metaFilePath));
	...
	public async Task<Unit> LoadProjectMetaAsync(string projectsFolderPath)
	{
		try
		{
				...
				else
				{
					...
					if (projectMetaFiles.Count > 0)
					{
						...
						foreach (var f in projectMetaFiles)
						{
							...
							if (match.Success)
							{
								var modUUID = match.Groups[1].Value;
			...
			if (Directory.Exists(projectDirectory))
			{
				...
				var projectMetaFileContents = XMLHelper.EscapeXmlAttributes(await FileCommands.ReadFileAsync(projectMetaFilePath));
				...
				if (!String.IsNullOrWhiteSpace(projectMetaFileContents))
				{
					...
					try
					{
						var projectMetaCreationTime = File.GetCreationTime(projectMetaFilePath);
						var projectMetaModifiedTime = File.GetLastWriteTime(projectMetaFilePath);
						var modMetaCreationTime = File.GetCreationTime(modMetaFilePath);

						var dates = new List<DateTime>() { projectMetaCreationTime, projectMetaModifiedTime, modMetaCreationTime };
						dates.Sort();
					...
					//Log.Here().Activity($"Set {this.DisplayName} creation date to: {ProjectInfo.CreationDate.ToString("d")}");
				}
	...
	public async Task<Unit> ReloadDataAsync()
	{
		Log.Here().Activity("Reloading mod data.");

		if (File.Exists(ModMetaFilePath))
		{
			var contents = await FileCommands.ReadFileAsync(ModMetaFilePath);
		...
		{
			var contents = XMLHelper.EscapeXmlAttributes(await FileCommands.ReadFileAsync(ProjectMetaFilePath));
			if (!String.IsNullOrWhiteSpace(contents))
			{
