...
public class DOS2DEModuleData : ModuleData<DOS2DESettingsData>
{
	...
	*/
	...
	public readonly SourceList<ModProjectData> ModProjects = new();
	...
	public RxCommandUnit RefreshAllCommand { get; internal set; }
	...
	public override string LoadStringResource(string Name)
	{
		return Name switch
		{
			nameof(Resources.DefaultGitIgnore) => Resources.DefaultGitIgnore,
			nameof(Resources.DefaultReadme) => Resources.DefaultReadme,
			nameof(Resources.DefaultChangelog) => Resources.DefaultChangelog,
			nameof(Resources.DefaultGitAttributes) => Resources.DefaultGitAttributes,
			_ => string.Empty,
		};
	}
	...
	public DOS2DEModuleData() : base(DOS2DE_ModuleDisplayName, DOS2DE_ModuleFolderName)
	{
		...
		OpenInLocalizationEditorCommand.ThrownExceptions.Subscribe((ex) =>
		{
			Log.Here().Error($"Error opening Localization Editor: {ex}");
