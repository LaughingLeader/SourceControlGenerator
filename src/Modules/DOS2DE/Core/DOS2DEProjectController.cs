...
public class DOS2DEProjectController : IProjectController
{
	...
	private async Task<int> OnGitGenerationAsync()
	{
		...
		var total = Data.GitGenerationSettings.ExportProjects.Count;
		...
		for (var i = 0; i < total; i++)
		{
			...
			var modProjectData = (ModProjectData)project;
				...
				foreach (var templateSetting in generationSettings.TemplateSettings)
				{
					...
					if (templateData != null)
					{
						if (templateSetting.Enabled)
						{
							...
							var outputText = GitGenerator.ReplaceKeywords(templateData.EditorText, modProject, MainAppData, Data);
				...
				else
				{
					switch (generationSettings.SelectedLicense)
					{
						case LicenseType.MIT:
							outputText = DefaultResources.License_MIT;
							break;

						case LicenseType.Apache:
							outputText = DefaultResources.License_Apache;
							break;

						case LicenseType.GPL:
							outputText = DefaultResources.License_GPL;
							break;
					}
				}
	...
	{
		var projectBackupDirectory = Data.Settings.BackupRootDirectory;
	...
	public async void StartBackupSelectedProjectsAsync()
	{
		...
		var selectedProjects = new ConcurrentBag<ModProjectData>(sortedProjects);
	...
	public async Task<FileCreationTaskResult> BackupProjectAsync(ModProjectData modProject, string OutputDirectory = "", BackupMode mode = BackupMode.Zip)
	{
		...
		var archiveName = modProject.ProjectFolder + "_" + DateTime.Now.ToString(sysFormat + "_HH-mm-ss") + ".zip";
		string archivePath = Path.Combine(OutputDirectory, archiveName);
		var gitProjectDirectory = "";
	...
	public async void StartPackageSelectedProjectsAsync()
	{
		...
		var successCount = allTaskResults.Where(x => x.Result != FileCreationTaskResult.Error).Count();
	...
	private async Task<List<FileCreationTaskData>> PackageSelectedProjectsAsync(ConcurrentBag<ModProjectData> selectedProjects, string targetFolder, bool finishProgressOnDone = true)
	{
		...
		[
			Path.Combine(Data.Settings.DOS2DEDataDirectory, @"Mods\ModFolder"),
			Path.Combine(Data.Settings.DOS2DEDataDirectory, @"Public\ModFolder"),
		];

		List<FileCreationTaskData> taskResults = [];
		...
		if (selectedProjects != null)
		{
			...
			foreach (var project in selectedProjects)
			{
				...
				var packageTask = await PackageProjectAsync(project, targetFolder, exportDirectories);
	...
	public List<string> IgnoredPackageFiles = [
		"ReConHistory.txt",
		"dialoglog.txt",
		"errors.txt",
		"log.txt",
	...
	public async Task<FileCreationTaskData> PackageProjectAsync(ModProjectData modProject, string outputDirectory, List<string> exportDirectories)
	{
		...
		var taskData = new FileCreationTaskData()
		{
	...
	public async void StartReleaseSelectedProjectsAsync()
	{
		...
		foreach (var task in targets)
		{
			...
			var zipName = mod.ProjectFolder.Replace(mod.UUID, "");
	...
	public void AddProjects(IEnumerable<string> selectedItems)
	{
		...
		foreach (var uuid in selectedItems)
		{
			var modData = Data.ModProjects.Items.FirstOrDefault(m => m.UUID == uuid);
			if (modData != null)
			{
	...
	public void LoadDirectoryLayout()
	{
		...
		{
			Data.Settings.DirectoryLayouts = [];
		}
		else
		{
		...
		else
		{
			...
			else
			{
				...
				layoutFileContents = Resources.DirectoryLayout;
			...
			using (var reader = new System.IO.StringReader(layoutFileContents))
			{
				for (var line = reader.ReadLine(); line != null; line = reader.ReadLine())
				{
					if (line.Trim().Length > 0)
					{
						...
						var offset = line.TakeWhile(c => char.IsWhiteSpace(c)).Count();
						var isComment = line.Substring(offset, 1) == "#";
	...
	public void OpenSetup(Action OnSetupFinished)
	{
		...
		{
			var setupWindow = new SetupWindow(this, OnSetupFinished)
			{
	...
	private readonly IObservable<bool> anySelected;
	...
	{
		var selectedProjects = Data.ManagedProjects.Where(x => x.Selected).ToList();
	...
	private void CreateEditorProjectFromPak_Start()
	{
		...
		var startDirectory = documentsFolder;
	...
	{
		var gamePaks = Directory.EnumerateFiles(gameDataPath, "*.pak", DefaultResources.FlatSearchOptions).ToList();
		var localeDirectory = Path.Combine(gameDataPath, "Localization");
		if (Directory.Exists(localeDirectory))
		{
			gamePaks.AddRange(Directory.EnumerateFiles(localeDirectory, "*.pak", DefaultResources.RecursiveOptions));
		}
		if (gamePaks.Count > 0)
		{
	...
	{
		var outputDirectory = Data.Settings.DOS2DEDataDirectory;
		...
		if (Directory.Exists(outputDirectory))
		{
			try
			{
				...
				using (var pak = reader.Read(path))
				{
					string pakName = Path.GetFileNameWithoutExtension(path);

					var metaFiles = pak.Files?.Where(pf => DOS2DEXMLUtils.IsModMetaFile(pakName, pf)).ToList()!;
					...
					if (metaFile != null)
					{
						...
						var metaContents = await sr.ReadToEndAsync();
				...
				if (moduleInfo != null && !String.IsNullOrWhiteSpace(moduleInfo.UUID) && !String.IsNullOrWhiteSpace(moduleInfo.Folder))
				{
					...
					if (!File.Exists(projectMetaFile))
					{
						...
						var file = new System.IO.FileInfo(projectMetaFile);
