...
public static class LocaleEditorCommands
{
	...
	public static HashSet<string> IgnoredHandles { get; set; } = [];
	...
	public static async Task<LocaleViewModel> LoadLocalizationDataAsync(LocaleViewModel localizationData, DOS2DEModuleData vm, ModProjectData modProject, CancellationToken? token = null)
	{
		...
		if (success)
		{
			...
			List<ILocaleKeyEntry> removedEntries = [];
	...
	public static async Task<bool> LoadProjectLocalizationDataAsync(LocaleViewModel localizationData, DOS2DEModuleData vm, ModProjectData modProjectData, CancellationToken? token = null)
	{
		try
		{
			...
			var dataRootPath = vm.Settings.DOS2DEDataDirectory;
	...
	{
		List<LocaleNodeFileData> stringKeyData = [];
		var lsbFiles = Directory.EnumerateFiles(directoryPath, "*", DefaultResources.RecursiveOptions).Where(x => FileCommands.FileExtensionFound(x, fileExtensions));
		var targetFiles = new ConcurrentBag<string>(lsbFiles);
		foreach (var filePath in targetFiles)
		{
			var isJournal = Path.GetFileName(Path.GetDirectoryName(filePath)) == "Journal";
	...
	{
		List<LocaleCustomFileData> customFiles = [];
	...
	private static async Task<LocaleCustomFileData?> LoadCustomFileAsync(string path)
	{
		try
		{
			var fileData = await JsonInterface.DeserializeObjectAsync<LocaleCustomFileData>(path);
	...
	public static LocaleNodeFileData? LoadResource(LocaleTabGroup groupData, string path, bool isJournal = false)
	{
		try
		{
			...
			var resource = ResourceUtils.LoadResource(path, resourceFormat, _resourceLoadSettings);
	...
	private static void LoadFromNode_Recursive(List<LocaleNodeKeyEntry> newEntries, Node node, ResourceFormat resourceFormat)
	{
		if (node.Attributes.TryGetValue("Type", out var nodeTypeAttribute))
		{
			...
			{
				var nodeType = (string)nodeTypeAttribute.Value;
	...
	private static void FindQuestNodes(string nodeName, Node node, List<LocaleNodeKeyEntry> newEntries)
	{
		...
		{
			if (kvp.Value.Type == AttributeType.FixedString)
			{
				var key = new LocaleNodeKeyEntry(node)
				{
					KeyIsEditable = false,
					Key = kvp.Key,

					TranslatedStringAttribute = kvp.Value,
					HandleIsEditable = false,
					Handle = nodeName
				};
				newEntries.Add(key);
			}
			else if (kvp.Value.Type == AttributeType.TranslatedString || kvp.Value.Type == AttributeType.TranslatedFSString)
			{
				var key = new LocaleNodeKeyEntry(node)
				{
					KeyIsEditable = false,
					Key = kvp.Key,
					TranslatedStringAttribute = kvp.Value,
					TranslatedString = kvp.Value.Value as TranslatedString
				};
				newEntries.Add(key);
			}
		}
	...
	{
		List<LocaleNodeKeyEntry> newEntries = [];
			...
			if (resourceFormat == ResourceFormat.LSB || resourceFormat == ResourceFormat.LSX)
			{
				...
				foreach (var entry in rootNode.Children)
				{
					foreach (var node in entry.Value)
					{
						...
						{
							var localeEntry = LoadFromNode(node, resourceFormat);
	...
	public static List<LocaleNodeKeyEntry> LoadAllFromNodeAttributes(Node node, ResourceFormat resourceFormat, bool generateNewHandle = false)
	{
		...
		foreach (var kvp in node.Attributes)
		{
			if (kvp.Value.Type == AttributeType.TranslatedString)
			{
				// || kvp.Value.Type == AttributeType.TranslatedFSString
				var localeEntry = new LocaleNodeKeyEntry(node)
				{
					KeyIsEditable = false,
					Key = kvp.Key,
	...
	public static LocaleNodeKeyEntry LoadFromNode(Node node, ResourceFormat resourceFormat, bool generateNewHandle = false)
	{
		if (resourceFormat == ResourceFormat.LSB || resourceFormat == ResourceFormat.LSX)
		{
			...
			{
				keyAtt = new NodeAttribute(AttributeType.FixedString);
				localeEntry.KeyAttribute = keyAtt;
			}
			if (!node.Attributes.TryGetValue("Content", out contentAtt))
			{
				contentAtt = new NodeAttribute(AttributeType.TranslatedString);
		...
		else if (resourceFormat == ResourceFormat.LSJ)
		{
			...
			{
				KeyIsEditable = false,
				Key = resourceFormat == ResourceFormat.LSJ ? "TagText" : ""
			};
	...
	public static async Task<bool> BackupDataFiles(LocaleViewModel data, string backupDirectory, CancellationToken? token = null)
	{
		try
		{
			...
			foreach (var f in data.SelectedGroup.DataFiles.OfType<LocaleNodeFileData>())
			{
				...
				{
					var regex = new Regex(@"(.*Divinity Original Sin 2\\DefEd\\Data\\)", RegexOptions.IgnoreCase);
	...
	public static async Task<bool> BackupDataFile(string sourceFilePath, string backupDirectory, CancellationToken? token = null)
	{
		try
		{
			if (File.Exists(sourceFilePath))
			{
				...
				List<string> sourceFiles = [sourceFilePath];
				var result = await BackupGenerator.CreateArchiveFromFiles(sourceFiles, archivePath, replacePaths, token);
				Log.Here().Activity($"Localization backup result to '{archivePath}': {result.ToString()}");
				return result != FileCreationTaskResult.Error;
			}
	...
	public static async Task<int> SaveDataFiles(LocaleViewModel data, CancellationToken? token = null)
	{
		...
		{
			foreach (var f in data.SelectedGroup.DataFiles.Where(f => f.ChangesUnsaved || f.Entries.Any(x => x.ChangesUnsaved)).Cast<LocaleNodeFileData>())
			{
				var result = await SaveDataFile(f, token);
				success += result;
				if (result > 0)
				{
		...
		else
		{
			foreach (var f in data.CustomGroup.DataFiles.Where(f => f.ChangesUnsaved).Cast<LocaleCustomFileData>())
			{
				...
				{
					var result = await SaveDataFile(f, targetDirectory, token);
					success += result;
					if (result > 0)
					{
						f.SetChangesUnsaved(false, true);
	...
	public static async Task<int> SaveDataFile(LocaleNodeFileData dataFile, CancellationToken? token = null)
	{
		try
		{
			if (dataFile.Source != null)
			{
				...
				var outputFile = dataFile.SourcePath;
	...
	public static async Task<int> SaveDataFile(LocaleCustomFileData dataFile, string targetDirectory, CancellationToken? token = null)
	{
		try
		{
			...
			{
				var output = "Key\tContent\tHandle\n";
				for (var i = 0; i < dataFile.Entries.Count; i++)
				{
			...
			else if (dataFile.SourcePath.EndsWith(".json") || string.IsNullOrEmpty(dataFile.SourcePath))
			{
				...
				var json = JsonInterface.SerializeObject(dataFile);
	...
	static string ProcessEntry(string sourcePath, ILocaleKeyEntry entry, ILocaleFileData fileData, bool exportSource, bool exportKeys, Dictionary<string, ILocaleKeyEntry> customKeys)
	{
		...
		var content = EscapeXml(entry.Content);
	...
	public static string ExportDataAsXML(LocaleViewModel data, bool exportAll = false, EnumLocaleLanguages language = EnumLocaleLanguages.None)
	{
		...
		if (data.SelectedGroup != null)
		{
			...
			if (fileData != null)
			{
				var findActualSource = fileData == data.SelectedGroup.CombinedEntries;

				List<ILocaleKeyEntry> entries = [];
				Dictionary<string, ILocaleKeyEntry> customKeys = [];
				...
				{
					var sourcePath = GetSourceFileName(fileData, findActualSource, data, e);
					var addStr = ProcessEntry(sourcePath, e, fileData, exportSource, exportKeys, customKeys);
				...
				{
					var sourcePath = GetSourceFileName(entry.Parent, findActualSource, data, entry);
					var addStr = ProcessEntry(sourcePath, entry, fileData, exportSource, exportKeys, customKeys);
					if (!String.IsNullOrWhiteSpace(addStr))
					{
						lines.Add(addStr);
					}
	...
	private static readonly ResourceLoadParameters _resourceLoadSettings = new()
	{
		ByteSwapGuids = false
	};
	...
	public static Resource CreateLocalizationResource()
	{
		try
		{
			using (var stream = new System.IO.MemoryStream())
			{
				...
				writer.Write(Resources.DefaultLocaleResource);
				writer.Flush();
				stream.Position = 0;
				Log.Here().Activity("Creating default localization resource.");
				var resource = ResourceUtils.LoadResource(stream, ResourceFormat.LSX, _resourceLoadSettings);
	...
	{
		var linkFolder = DOS2DEDefaultPaths.LocalizationEditorLinkFolder(vm);
		if (Directory.Exists(linkFolder))
		{
			Log.Here().Activity($"Searching for link files in '{linkFolder}'.");

			var linkFiles = Directory.EnumerateFiles(linkFolder, ".json", DefaultResources.RecursiveOptions);
	...
	public static void SaveLinkedData(DOS2DEModuleData moduleData, LocaleProjectLinkData linkFile)
	{
		...
		if (!string.IsNullOrEmpty(filePath))
		{
			...
			var json = JsonInterface.SerializeObject(linkFile);
	...
	{
		List<ILocaleKeyEntry> allMissingEntries = [];
		if (File.Exists(fileData.FileLinkData.ReadFrom))
		{
			...
			var path = fileData.FileLinkData.ReadFrom;
			...
			if (FileCommands.FileExtensionFound(path, ".txt", ".tsv", ".csv"))
			{
				...
				using (var stream = new System.IO.StreamReader(path))
				{
					...
					List<TextualLocaleEntry> entries = [];

					var regularModePattern = new Regex($"^({notDelimiter}*){delimiter}+(.*)", RegexOptions.Singleline);
					var handleModePattern = new Regex($"^({notDelimiter}*?){delimiter}+({notDelimiter}*){delimiter}+({notDelimiter}*?)", RegexOptions.Singleline);

					var r = regularModePattern;
					...
					while ((line = stream.ReadLine()) != null)
					{
						...
						if (match.Success)
						{
							var key = match.Groups.Count >= 1 ? match.Groups[1].Value : "NewKey";
							var content = match.Groups.Count >= 2 ? match.Groups[2].Value : "";
							var handle = handleMode && match.Groups.Count >= 3 ? match.Groups[3].Value : "";
	...
	public static LocaleNodeKeyEntry CreateNewLocaleEntry(ILocaleFileData fileData, string key = "NewKey", string content = "")
	{
		...
		if (parent == null)
		{
			...
			{
				Name = "root",
				RegionName = "TranslatedStringKeys"
			};
		}
		...
		{
			Parent = parent,
			Name = "TranslatedStringKey"
		};
		parent.AppendChild(node);

		node.Attributes = [];
		var translatedStringAtt = new NodeAttribute(AttributeType.TranslatedString);
		...
		var keyAtt = new NodeAttribute(AttributeType.FixedString)
		{
			Value = key == "NewKey" ? key + (fileData.Entries.Count + 1) : key
		};

		node.Attributes.Add("Content", translatedStringAtt);
		node.Attributes.Add("ExtraData", new NodeAttribute(AttributeType.LSString) { Value = "" });
		node.Attributes.Add("Speaker", new NodeAttribute(AttributeType.FixedString) { Value = "" });
		node.Attributes.Add("Stub", new NodeAttribute(AttributeType.Bool) { Value = false });
		node.Attributes.Add("UUID", keyAtt);

		var localeEntry = LoadFromNode(node, format);
	...
	public static Dictionary<string, int> GetSheetParamOrder(string line, char delimiter)
	{
		...
		Dictionary<string, int> paramOrder = [];
	...
	{
		if (fileParameters.TryGetValue(paramName, out var index))
		{
	...
	private static LocaleNodeFileData CreateNodeFileDataFromTextual(LocaleTabGroup groupData, System.IO.StreamReader stream, string sourceDirectory, string filePath, char delimiter)
	{
		...
		var futureSourcePath = FileCommands.EnsureExtension(Path.Combine(sourceDirectory, name), ".lsb");

		var fileData = CreateFileData(groupData, futureSourcePath, name);
		...
		while ((line = stream.ReadLine()) != null)
		{
			...
			else if (!string.IsNullOrWhiteSpace(line) && fileParameters.Count > 0)
			{
				...
				var key = GetSheetValue("key", fileParameters, lineEntries);
				var content = GetSheetValue("content", fileParameters, lineEntries);
				var handle = GetSheetValue("handle", fileParameters, lineEntries);
	...
	{
		List<ILocaleFileData> newFileDataList = [];
		foreach (var path in files)
		{
			if (FileCommands.FileExtensionFound(path, ".lsb", ".lsj", ".lsx"))
			{
				var task = Task.Run<ILocaleFileData>(async () => await LoadResourceAsync(groupData, path, groupData.Name == "Journal"));
	...
	{
		List<ILocaleKeyEntry> newEntryList = [];
		try
		{
			foreach (var path in files)
			{
				...
				if (FileCommands.FileExtensionFound(path, ".lsb", ".lsj"))
				{
					...
					var isJournal = Path.GetFileName(Path.GetDirectoryName(path)) == "Journal";
					var task = Task.Run<ILocaleFileData>(async () => await LoadResourceAsync(fileData.Parent, path, isJournal).ConfigureAwait(false));
					if (task != null && task.Result != null)
					{
	...
	public static void SaveSettings(DOS2DEModuleData moduleData, LocaleViewModel localeData)
	{
		...
		if (localeData.Settings != null)
		{
			...
			var json = JsonInterface.SerializeObject(localeData.Settings);
	...
	public static void Debug_CreateEntries(ILocaleFileData fileData, ObservableCollectionExtended<ILocaleKeyEntry> Entries)
	{
		...
		{
			var node = new Node();
			var att = new NodeAttribute(AttributeType.TranslatedString)
			{
	...
	public static void Debug_CreateCustomEntries(ILocaleFileData fileData, ObservableCollectionExtended<ILocaleKeyEntry> Entries)
	{
		...
		for (var i = 0; i < 4; i++)
		{
			var data = new LocaleCustomKeyEntry(fileData)
			{
