...
public class AppController
{
	...
	public void LoadAppSettings()
	{
		if (File.Exists(DefaultPaths.MainAppSettingsFile))
		{
			try
			{
				...
				var json = FileCommands.ReadFile(DefaultPaths.MainAppSettingsFile);
	...
	public void MenuAction_AddNewTemplate()
	{
		...
		if (CurrentModule != null)
		{
			...
			{
				var mainTabs = (TabControl)mainWindow.FindName("MainTabsControl");
	...
	public void MenuAction_SaveLog()
	{
		...
		var fileName = "SourceControlGenerator_Log_" + DateTime.Now.ToString(sysFormat + "_HH-mm") + ".txt";
	...
	{
		var listView = (ListView)mainWindow.FindName("ModuleSettingsListView");
		if (listView != null)
		{
	...
	public void OpenProjectReadmeInMarkdownConverter(object obj)
	{
		...
		if (obj is IProjectData projectData)
		{
			...
			else
			{
				...
				var projectFiles = Directory.EnumerateFiles(CurrentModule.ModuleData.ModuleSettings.GitRootDirectory, "SourceControlGenerator.json", DefaultResources.RecursiveOptions)
	...
	public void SaveKeywordsAs()
	{
		if (Data.CurrentModuleData != null && Data.CurrentModuleData.ModuleSettings != null)
		{
			var json = JsonConvert.SerializeObject(Data.CurrentModuleData.UserKeywords, Newtonsoft.Json.Formatting.Indented);
	...
	public string SaveTextGeneratorData()
	{
		...
		try
		{
			...
			{
				var msg = $"TextGenerator settings were saved to '{outputDataPath}'.";
				Log.Here().Activity(msg);
				return msg;
			}
			else
			{
				var msg = $"Error saving TextGenerator settings to '{outputDataPath}'.";
				Log.Here().Error(msg);
				return msg;
			}
		}
	...
	public AppController(MainWindow MainAppWindow)
	{
		...
		{
			SaveKeywordsCommand = new ActionCommand(SaveKeywords),
			SaveKeywordsAsCommand = new ActionCommand(SaveKeywordsAs)
		};
