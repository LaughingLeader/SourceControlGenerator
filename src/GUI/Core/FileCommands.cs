...
public static class FileCommands
{
	...
	public static string ReadFile(string filePath)
	{
		...
		try
		{
			...
			{
				using (var stream = new System.IO.FileStream(filePath, System.IO.FileMode.Open, System.IO.FileAccess.Read, System.IO.FileShare.Read, 4096, false))
				{
	...
	{
		using (var stream = new System.IO.FileStream(filePath, System.IO.FileMode.Open, System.IO.FileAccess.Read, System.IO.FileShare.Read, 4096, true))
		{
			using (var sr = new System.IO.StreamReader(stream))
			{
	...
	public static bool WriteToFile(string filePath, string contents, bool supressLogMessage = true)
	{
		try
		{
			...
			using (var stream = new System.IO.FileStream(filePath, System.IO.FileMode.Create, System.IO.FileAccess.Write, System.IO.FileShare.Write, 4096, false))
			{
	...
	public static async Task<bool> WriteToFileAsync(string filePath, string contents, bool supressLogMessage = true)
	{
		try
		{
			...
			using (var stream = new System.IO.FileStream(filePath, System.IO.FileMode.Create, System.IO.FileAccess.Write, System.IO.FileShare.Write, 4096, true))
			{
				using (var sw = new System.IO.StreamWriter(stream))
				{
	...
	public static void OpenConfirmationDialog(Window ParentWindow, string WindowTitle, string MainInstruction, string Content, Action<bool> TaskAction)
	{
		if (TaskDialog.OSSupportsTaskDialogs)
		{
			using (var dialog = new TaskDialog())
			{
				...
				var button = dialog.ShowDialog(ParentWindow);
	...
	*/
	...
	public static bool FileExtensionFound(string fPath, params string[] extensions)
	{
		if (extensions.Length > 1)
		{
			...
			var result = Array.BinarySearch(extensions, Path.GetExtension(fPath), StringComparer.OrdinalIgnoreCase);
	...
	public static bool FileExists(string path)
	{
		...
		{
			return File.Exists(Path.GetRelativePath(AppDirectory, path));
		}
		else
		{
	...
	public static bool DirectoryExists(string path)
	{
		if (PathIsRelative(path))
		{
			return Directory.Exists(Path.GetRelativePath(AppDirectory, path));
		}
		else
		{
