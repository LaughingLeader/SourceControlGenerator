...
public class LoadCommands
{
	...
	*/
	...
	private OpenFileDialog CreateOpenDialog(string title, string startingDirectory, string defaultFilename = "", bool multiSelect = false, params FileBrowserFilter[] filters)
	{
	...
	public void OpenFileDialog(Window parentWindow, string title, string startingDirectory, Action<string> OnFileSelected, string defaultFilename = "", Action<string, FileDialogResult> OnCancel = null, params FileBrowserFilter[] filters)
	{
		...
		{
			OnCancel?.Invoke(fileDialog.InitialDirectory, result == false ? FileDialogResult.Cancel : FileDialogResult.None);
		}
	}
	...
	*/
	...
	public void OpenFolderDialog(Window ParentWindow, string Title, string FilePath, Action<string> OnFolderSelected, bool RetainRelativity = true)
	{
		var dialog = new Ookii.Dialogs.Wpf.VistaFolderBrowserDialog()
		{
			...
			SelectedPath = !String.IsNullOrEmpty(FilePath) ? Path.GetFullPath(FilePath) : Path.GetFullPath(Assembly.GetExecutingAssembly().Location)
		};
	...
	public void SelectFoldersDialog(Window ParentWindow, string Title, string FilePath, Action<List<string>> OnFoldersSelected)
	{
		...
		if (result == true)
		{
			...
			foreach (var path in dialog.SelectedPaths)
			{
				if (FileCommands.PathIsRelative(path))
				{
					folders.Add(path.Replace(Directory.GetCurrentDirectory(), ""));
				}
				else
				{
					folders.Add(path);
	...
	{
		var templateFilePath = DefaultPaths.ModuleTemplateSettingsFile(Data);
		...
		{
			FileCommands.WriteToFile(templateFilePath, DefaultResources.Templates);
		}
		...
		try
		{
			...
			{
				var templateData = TemplateEditorData.LoadFromXml(Data, template);
	...
	{
		var filePath = DefaultPaths.ModuleGitGenSettingsFile(Data);
		...
		ObservableImmutableList<TemplateGenerationData> templateSettings = [];
		foreach (var template in Data.Templates.Where(t => t.ID.ToLower() != "license"))
		{
			var tdata = new TemplateGenerationData()
			{
				ID = template.ID,
	...
	public SourceControlData LoadSourceControlData(string filePath)
	{
		if (!String.IsNullOrEmpty(filePath) && File.Exists(filePath))
		{
			try
			{
				var data = JsonConvert.DeserializeObject<SourceControlData>(File.ReadAllText(filePath));
	...
	public async Task<SourceControlData> LoadSourceControlDataAsync(string filePath)
	{
		if (!String.IsNullOrEmpty(filePath) && File.Exists(filePath))
		{
			try
			{
				...
				var data = JsonConvert.DeserializeObject<SourceControlData>(contents);
				data.RepositoryPath = Directory.GetDirectoryRoot(filePath);
				return data;
			}
			catch (Exception ex)
