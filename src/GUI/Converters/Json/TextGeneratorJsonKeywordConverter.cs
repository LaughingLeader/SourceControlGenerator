...
public class TextGeneratorJsonKeywordConverter : JsonConverter
{
	...
	{
		var jObjectReader = obj.CreateReader();
	...
	public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
	{
		...
		var inputTypeString = (string)obj.SelectToken("InputType");

		if (!String.IsNullOrWhiteSpace(inputTypeString))
		{
			var inputType = (TextGeneratorInputType)Enum.Parse(typeof(TextGeneratorInputType), inputTypeString);
			...
			{
				var target = new TextGeneratorInputNumberData();
			...
			{
				var target = new TextGeneratorInputTextData();
		...
		{
			var vm = new TextGeneratorViewModel();
			var jObjectReader = CreateNewReader(reader, obj);
