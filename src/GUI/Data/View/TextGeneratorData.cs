...
public class TextGeneratorViewModel : ReactiveObject
{
	...
	public int ActivePresetIndex
	{
		...
		set
		{
			var bSaveData = textGeneratorActiveSettingsIndex != value;
...
public class TextGeneratorData : ReactiveObject
{
	...
	public int GenerationAmount
	{
		...
		set
		{
			var bSaveData = generationAmount != value;
	...
	public TextGeneratorInputType NextKeywordType
	{
		...
		set
		{
			var bSaveData = nextKeywordType != value;
	...
	public void Init(TextGeneratorViewModel parent)
	{
		...
		var initialDirectory = AppController.Main.CurrentModule != null ? DefaultPaths.ModuleTextGeneratorFolder(AppController.Main.CurrentModule.ModuleData) : DefaultPaths.RootFolder + @"Default\TextGenerator\";
...
{
	private readonly TextGeneratorInputType inputType = TextGeneratorInputType.Text;
...
public class TextGeneratorInputNumberData : ReactiveObject, ITextGeneratorInputData
{
	...
	public string GetOutput(string input)
	{
		...
		var finalPadding = NumberPadding > 0 ? NumberPadding + 1 : 0;
		var finalResult = result.ToString($"D{finalPadding}");
